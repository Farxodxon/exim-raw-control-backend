...
void main() async {
  ...
  try {
    ...
    try {
      ...
      // users.role CHECK — 'employee' rolini qo'shish.
      await conn.execute(r'''
        ...
        BEGIN
            ...
            IF constraint_name IS NOT NULL THEN
                EXECUTE 'ALTER TABLE fh.users DROP CONSTRAINT ' || constraint_name;
        ...
        END $$;
      ''');
      print('✅ fh.users.department column + employee role ensured');
    } catch (_) {}
    try {
      final conn = await DatabaseConnection.getConnection();
    ...
    try {
      ...
      ''');
      await conn.execute("ALTER TABLE fh.attendance DROP CONSTRAINT IF EXISTS attendance_marked_by_check");
      await conn.execute("ALTER TABLE fh.attendance ADD CONSTRAINT attendance_marked_by_check CHECK (marked_by IN ('self', 'manager'))");
      await conn.execute('''
      ...
      ''');
      await conn.execute('CREATE INDEX IF NOT EXISTS idx_attendance_audit_att ON fh.attendance_audit_log(attendance_id)');
      await conn.execute('''
      ...
      // monthly_payroll_summary view — days_early_leave ustunini qo'shish.
      await conn.execute('''
...
LEFT JOIN adj_agg  adj  ON adj.employee_id = m.employee_id AND adj.month = m.month;
  ...
  router.get('/api/products', (Request request) async {
    try {
      ...
      final products = result.map((row) => {
        ...
        'composition_text': row[18]?.toString(),
      }).toList();
  ...
  router.post('/api/products', (Request request) async {
    try {
      ...
      final insertResult = await conn.execute(
        ...
        'category_id, box_length_cm, box_width_cm, box_height_cm, box_netto_kg, box_brutto_kg, composition_text) '
        ...
        parameters: [body['barcode'], body['name'], body['category'], body['tnved'],
          ...
          body['box_netto_kg'], body['box_brutto_kg'], body['composition_text']],
      );
  ...
  router.put('/api/products/<id>', (Request request, String id) async {
    try {
      ...
      final updateResult = await conn.execute(
        ...
        'composition_text=\$14, updated_at=NOW() WHERE id=\$15 RETURNING id',
        parameters: [body['barcode'], body['name'], body['category'], body['tnved'],
          body['pcs_in_box'], body['price_usd'], body['netto_per_piece'],
          body['category_id'], body['box_length_cm'], body['box_width_cm'], body['box_height_cm'],
          body['box_netto_kg'], body['box_brutto_kg'], body['composition_text'], int.parse(id)],
      );
    ...
    try {
      ...
      await conn.execute('''
        CREATE TABLE IF NOT EXISTS partners (
          id SERIAL PRIMARY KEY,
          firma_nomi VARCHAR(200) NOT NULL,
          firma_turi VARCHAR(150),
          shartnoma_raqami VARCHAR(100),
          davlati VARCHAR(100),
          faolligi BOOLEAN NOT NULL DEFAULT true,
  ...
  // POST /api/partners
  router.post('/api/partners', (Request request) async {
    try {
      final body = jsonDecode(await request.readAsString());
      final conn = await DatabaseConnection.getConnection();
      final insertResult = await conn.execute(
  ...
  // PUT /api/partners/:id
  router.put('/api/partners/<id>', (Request request, String id) async {
    try {
      final body = jsonDecode(await request.readAsString());
      final conn = await DatabaseConnection.getConnection();
      final updateResult = await conn.execute(
  ...
  router.delete('/api/partners/<id>', (Request request, String id) async {
    try {
      final conn = await DatabaseConnection.getConnection();
